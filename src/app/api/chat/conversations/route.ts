...
export async function GET(req: NextRequest) {
  ...
  const { data: convs } = await getAdminClient()
    .from("conversations")
  ...
  const results = await Promise.all(convs.map(async (c) => {
    ...
    const { data: lastMsg } = await getAdminClient()
...
export async function POST(req: NextRequest) {
  ...
  const { data: target } = await getAdminClient().from("profiles").select("id").eq("username", username).single();
  ...
  if (settings?.can_message === "friends") {
    const { data: fr } = await getAdminClient().from("friendships").select("id").eq("status", "accepted")
  ...
  const { data: conv, error } = await getAdminClient()
    .from("conversations")
    .upsert({ user1_id: u1, user2_id: u2 }, { onConflict: "user1_id,user2_id" })
    .select("id").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  const { data: otherProfile } = await getAdminClient().from("profiles")
