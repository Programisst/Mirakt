...
export async function GET(req: NextRequest) {
  ...
  const { data } = await getAdminClient().from("chat_settings").select("can_message").eq("user_id", user.id).maybeSingle();
...
export async function PATCH(req: NextRequest) {
  ...
  await getAdminClient().from("chat_settings").upsert({ user_id: user.id, can_message }, { onConflict: "user_id" });
